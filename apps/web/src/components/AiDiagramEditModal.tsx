...
import { AiDiagramPatchPreview } from './AiDiagramPatchPreview';
...
export function AiDiagramEditModal({ isOpen, onClose }: AiDiagramEditModalProps) {
  ...
  useEffect(() => {
    function handleClickOutside(event: MouseEvent) {
      if (modalRef.current && !modalRef.current.contains(event.target as Node)) {
        if (!isGenerating && !isApplying) onClose();
      }
    }
  ...
  }, [isOpen, isGenerating, isApplying, onClose]);

  // Reset instruction when closing
  useEffect(() => {
    if (!isOpen) {
      setInstruction('');
      clearEdit(); // this clears error, clarification, and preparedEdit
    }
  }, [isOpen, clearEdit]);
  ...
  const handleGenerate = async () => {
    ...
    await prepareEdit(instruction);
  };

  const handleApply = async () => {
    const success = await applyEdit();
    if (success) {
      setInstruction('');
      onClose();
    }
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
    if (e.key === 'Enter' && (e.metaKey || e.ctrlKey)) {
      e.preventDefault();
      if (!isGenerating && instruction.trim() && !preparedEdit) {
        handleGenerate();
      }
    } else if (e.key === 'Escape' && !isGenerating && !isApplying) {
      e.preventDefault();
      if (preparedEdit) {
        clearEdit();
      } else {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <div 
        ref={modalRef}
        className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-2xl p-6 w-full max-w-lg shadow-2xl flex flex-col gap-4 transition-all"
        role="dialog"
      ...
      >
        <div className="flex justify-between items-center">
          <h3 id="ai-edit-title" className="text-xl font-semibold text-neutral-900 dark:text-neutral-100">
            {preparedEdit ? 'Review AI Changes' : 'Edit Diagram with AI'}
          </h3>
          <button
            onClick={() => {
              if (preparedEdit) {
        ...
        {preparedEdit ? (
          <AiDiagramPatchPreview 
            preview={preparedEdit.preview}
          ...
          <>
            <div className="flex flex-col gap-2">
              ...
              <div className="text-xs text-neutral-500 text-right">
                Press <kbd className="font-sans px-1 py-0.5 rounded bg-neutral-100 dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700">Cmd</kbd> + <kbd className="font-sans px-1 py-0.5 rounded bg-neutral-100 dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700">Enter</kbd> to generate
              </div>
            </div>
